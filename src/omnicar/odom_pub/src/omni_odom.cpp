...
void publish_odometry_msg(const boost::shared_ptr<std_msgs::Int32MultiArray const> msg, odom_msgs& odom_adv)
{
    ...
    if( abs(odom_adv.cur_encoder_msg.steer_motor_01) < 0.008 && 
        ...
        double dx = position * cos(odom_adv.cur_pos.pos_th);
        double dy = position * sin(odom_adv.cur_pos.pos_th);
        odom_adv.cur_pos.pos_x += dx;  // 这里的th需要是弧度
        odom_adv.cur_pos.pos_y += dy;
        ...
        odom_adv.cur_pos.linear_x = (dx / dt);
        odom_adv.cur_pos.linear_y = (dy / dt);
    ...
    else if(odom_adv.cur_encoder_msg.steer_motor_01 > -0.95 &&  
            odom_adv.cur_encoder_msg.steer_motor_01 < -0.92 &&
            odom_adv.cur_encoder_msg.steer_motor_02 < 0.95 &&  
            odom_adv.cur_encoder_msg.steer_motor_02 > 0.92 &&
            odom_adv.cur_encoder_msg.steer_motor_03 > -0.95 &&  
            odom_adv.cur_encoder_msg.steer_motor_03 < -0.92 &&
            odom_adv.cur_encoder_msg.steer_motor_04 < 0.95 &&  
            odom_adv.cur_encoder_msg.steer_motor_04 > 0.92 
            )
    {
    ...
    // 横行
    else if(odom_adv.cur_encoder_msg.steer_motor_01 > 1.56 &&  
            odom_adv.cur_encoder_msg.steer_motor_01 < 1.571 &&
            odom_adv.cur_encoder_msg.steer_motor_02 > 1.56 &&  
            odom_adv.cur_encoder_msg.steer_motor_02 < 1.571 &&
            odom_adv.cur_encoder_msg.steer_motor_03 > 1.56 &&  
            odom_adv.cur_encoder_msg.steer_motor_03 < 1.571 &&
            odom_adv.cur_encoder_msg.steer_motor_04 > 1.56 &&  
            odom_adv.cur_encoder_msg.steer_motor_04 < 1.571 )
    {
        double velocity = (odom_adv.cur_motor_v.driver_motor_01_v + odom_adv.cur_motor_v.driver_motor_02_v +
                            odom_adv.cur_motor_v.driver_motor_03_v + odom_adv.cur_motor_v.driver_motor_04_v) / 4;
        std::cout << "横行：velocity = " << velocity << std::endl;
        // 平均路程
        double position = velocity * dt;    // 这段时间的路程
        std::cout << "position = " << position << std::endl;
        // 坐标是相对于map的，正负方向不会因为车的转向而改变
        double dx = position * sin(-1 * odom_adv.cur_pos.pos_th);
        double dy = position * cos(odom_adv.cur_pos.pos_th);
        odom_adv.cur_pos.pos_x += dx;  // 这里的th需要是弧度
        odom_adv.cur_pos.pos_y += dy;
        odom_adv.cur_pos.pos_th += 0.0;
        // //判断速度方向 //这里有问题
    ...
    // 斜线模式
    else if(abs(odom_adv.cur_encoder_msg.steer_motor_01 - odom_adv.cur_encoder_msg.steer_motor_02) < 0.02 &&
            abs(odom_adv.cur_encoder_msg.steer_motor_02 - odom_adv.cur_encoder_msg.steer_motor_03) < 0.02 &&
            abs(odom_adv.cur_encoder_msg.steer_motor_03 - odom_adv.cur_encoder_msg.steer_motor_04) < 0.02 &&
            abs(odom_adv.cur_encoder_msg.steer_motor_04 - odom_adv.cur_encoder_msg.steer_motor_01) < 0.02 
            ) 
    {
        double theta = (odom_adv.cur_encoder_msg.steer_motor_01 + odom_adv.cur_encoder_msg.steer_motor_02 +
                        odom_adv.cur_encoder_msg.steer_motor_03 + odom_adv.cur_encoder_msg.steer_motor_04) / 4;
        double velocity = (odom_adv.cur_motor_v.driver_motor_01_v + odom_adv.cur_motor_v.driver_motor_02_v +
                            odom_adv.cur_motor_v.driver_motor_03_v + odom_adv.cur_motor_v.driver_motor_04_v) / 4;
        std::cout << "斜线模式：velocity = " << velocity << ",   theta = " << theta * 57.29 << std::endl;
        // 平均路程
        double position = velocity * dt;    // 这段时间的路程
        std::cout << "position = " << position << std::endl;
        // 坐标是相对于map的，正负方向不会因为车的转向而改变
        double dx = position * sin(odom_adv.cur_pos.pos_th + theta);
        double dy = position * cos(odom_adv.cur_pos.pos_th + theta);
        odom_adv.cur_pos.pos_x += dx;  // 这里的th需要是弧度
        odom_adv.cur_pos.pos_y += dy;
        odom_adv.cur_pos.pos_th += 0.0;
        // //判断速度方向 //这里有问题
    ...
    // 3、不走直线运动,在自由模式下
        ...
        double vel_1_x = odom_adv.cur_encoder_msg.driver_motor_01 * cos(odom_adv.cur_encoder_msg.steer_motor_01);
        double vel_1_y = odom_adv.cur_encoder_msg.driver_motor_01 * sin(odom_adv.cur_encoder_msg.steer_motor_01);

        double vel_2_x = odom_adv.cur_encoder_msg.driver_motor_02 * cos(odom_adv.cur_encoder_msg.steer_motor_02);
        double vel_2_y = odom_adv.cur_encoder_msg.driver_motor_02 * sin(odom_adv.cur_encoder_msg.steer_motor_02);
        
        double vel_3_x = odom_adv.cur_encoder_msg.driver_motor_03 * cos(odom_adv.cur_encoder_msg.steer_motor_03);
        double vel_3_y = odom_adv.cur_encoder_msg.driver_motor_03 * sin(odom_adv.cur_encoder_msg.steer_motor_03);

        double vel_4_x = odom_adv.cur_encoder_msg.driver_motor_04 * cos(odom_adv.cur_encoder_msg.steer_motor_04);
        double vel_4_y = odom_adv.cur_encoder_msg.driver_motor_04 * sin(odom_adv.cur_encoder_msg.steer_motor_04);
