...
class AmclNode
{
  ...
  private:
    ...
    // lasers_记录当前构建的雷达对象，lasers_update_标记雷达的更新状态
    // 并通过一个map建立雷达坐标在lasers_中的对应关系
...
boost::shared_ptr<AmclNode> amcl_node_ptr;  // 智能指针指向一个新的AmclNode对象

void sigintHandler(int sig)
{
  // Save latest pose as we're shutting down.
  amcl_node_ptr->savePoseToServer();  // 保存当前位姿到服务器上，然后退出ros
  ros::shutdown();
}
...
main(int argc, char** argv)
{
  ...
  signal(SIGINT, sigintHandler);  // ctrl-c的信号，响应函数中只是保存当前位姿到服务器上
  ...
  else if ((argc >= 3) && (std::string(argv[1]) == "--run-from-bag"))
  {
    if (argc == 3)
    {
      amcl_node_ptr->runFromBag(argv[2]); // 回放数据
    }
  ...
  amcl_node_ptr.reset();  // 当循环退出时，需要释放指针
...
{
  // 先对configuration_mutex_加锁，防止与动态参数配置冲突
  boost::recursive_mutex::scoped_lock l(configuration_mutex_);

  // Grab params off the param server
  // 通过private_nh_局部句柄获取amcl的运行参数
  ...
  updatePoseFromServer(); // 设置初始位姿和相应的方差
  ...
  // 发布amcl_pose主题，用于输出机器人的位姿
  pose_pub_ = nh_.advertise<geometry_msgs::PoseWithCovarianceStamped>("amcl_pose", 2, true);
  // 发布particlecloud主题，用于输出粒子集合
  particlecloud_pub_ = nh_.advertise<geometry_msgs::PoseArray>("particlecloud", 2, true);
  // 注册服务，用于获取机器人的全局定位
  global_loc_srv_ = nh_.advertiseService("global_localization", 
					 &AmclNode::globalLocalizationCallback,
                                         this);
  // 注册服务，用于手动触发粒子更新并发布新的位姿估计
  nomotion_update_srv_= nh_.advertiseService("request_nomotion_update", &AmclNode::nomotionUpdateCallback, this);
  // 注册服务，用于设定机器人的位姿和地图信息
  set_map_srv_= nh_.advertiseService("set_map", &AmclNode::setMapCallback, this);
  // 构建激光的消息过滤器，
  laser_scan_sub_ = new message_filters::Subscriber<sensor_msgs::LaserScan>(nh_, scan_topic_, 100);
  // 构建tf2的过滤器
  // 这里的message_filter为ros系统提供了一些通用的消息过滤方法，对接受到的消息进行缓存，
  // 只有当满足过滤条件后才输出，这里就是同时监听激光扫描消息和里程计坐标变换，同步二者的输出。
  laser_scan_filter_ = 
  ...
  if(use_map_topic_) {
    ...
    requestMap(); //  通过requestMap请求‘static_map’服务
  }
  ...
  // 设置一个15s定时器，见来检查激光雷达的接收数据,如果15s内没有新的数据就给出警告
...
AmclNode::checkLaserReceived(const ros::TimerEvent& event)
{
  ...
  if(d > laser_check_interval_) // >15s的话就超时
   {
...
// 主题‘map’的回调函数mapReceived，以及请求’static_map‘服务的requestMap在接收到地图
// 数据后，都会调用这个函数进一步完成AmclNode的初始化工作
/*
  以地图数据消息的引用为输入参数构建了地图对象，里程计对象和激光雷达传感器对象，
  并完成滤波器的初始化。
...
AmclNode::handleMapMessage(const nav_msgs::OccupancyGrid& msg)
{
  ...
  // 释放与地图相关的内存和对象，通过convertMap将地图消息转换成amcl的地图数据结构
  ...
  // 在接收到地图数据之前，有可能已经接收到机器人的初始位姿，调用这个函数来处理这个情况
  applyInitialPose();
...
{
  map_t* map = map_alloc(); // 构建map_t对象
  ROS_ASSERT(map);

  // 根据接收到的地图消息设定对象map的尺寸、原点和分辨率
  ...
  // 为对象的栅格单元申请内存，并且填充占用栅格
  map->cells = (map_cell_t*)malloc(sizeof(map_cell_t)*map->size_x*map->size_y);
  ROS_ASSERT(map->cells);
  for(int i=0;i<map->size_x * map->size_y;i++)
  {
    if(map_msg.data[i] == 0)
      map->cells[i].occ_state = -1; // -1是未被占用，0是不知道，1是被占用
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  // 通过在frame_to_laser中查找雷达的坐标ID，如果没有收到该雷达的消息，将新建一个对象保存
  // 在laser_中，并相应的在laser_update_中添加对应更新状态，建立映射关系
  // 如果存在。就直接通过frame_to_laser获取雷达对象的索引。
  // 这个应该是对应的多激光雷达的情况，我这里的laser_scan_frame_id应该就是laser
  if(frame_to_laser_.find(laser_scan_frame_id) == frame_to_laser_.end())
  {
    ...
    //如果已经存在这个雷达对象的话，就直接查找对应的index
    laser_index = frame_to_laser_[laser_scan_frame_id];
  }

  // Where was the robot when this scan was taken?
  // 获取接收到雷达数据时刻的里程计位姿，如果无法获得就报错退出
  // odom获得的位姿作为先验位姿
  pf_vector_t pose;
  ...
  if(pf_init_)
  {
    // 如果里程计的数据显示机器人已经发生了明显的位移或者旋转，就标记所的雷达更新标记为true;
    ...
    if(update) {
      std::cout << "delta.v[0] = " << delta.v[0] << ",  delta.v[1] = " << delta.v[1] << ",  delta.v[2] = " << delta.v[2] << std::endl;
    }
  ...
  // 如果需要更新，就调用里程计对象的UpdateAction接口完成运动更新
  else if(pf_init_ && lasers_update_[laser_index])
  {
    ...
    // 更新所有粒子的位置，用运动模型更新粒子位置，得到当前时刻的先验位置
  ...
  if(lasers_update_[laser_index])
  {
    ...
    {
      // 按照一定的规则进行重采样，包括失效恢复，粒子权重等，然后放到kdtree中
      // 然后进行聚类，得到均值和方差信息，就是将详尽的一堆粒子融合成一个平均值位置
      pf_update_resample(pf_);
      resampled = true;
    }
    
    // amcl的主业逻辑到这儿基本结束，剩下来的就是获取当前的粒子集合，从中提炼出机器人的位姿
    // 估计，并将这些消息通过各个主题发送出去。
  ...
  if(resampled || force_publication)
  {
    ...
    // 遍历所有的粒子簇，找到权重最大的那个簇，其平均位置就是机器人的后验位置，完成一次定位
    for(int hyp_count = 0;
    ...
    if(max_weight > 0.0)
    {
      ...
      pose_pub_.publish(p); // 发布权重最大簇的位置，就是这一次定位的结果
