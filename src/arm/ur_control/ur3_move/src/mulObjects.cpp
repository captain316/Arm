...
int main(int argc, char **argv)  //主函数
{
    ...
    ros::ServiceClient client = n.serviceClient<ur3_move::mulObjectsPosition>("getObjectPose");
    ur3_move::mulObjectsPosition getPose;
    ...
    for(int i = 0; i < getPose.response.targets_pose.size(); ++i) {
        ...
        double py = getPose.response.targets_pose[i].position.y;
        double pz = getPose.response.targets_pose[i].position.z + 0.045;
        ...
        if(first) {
            cartesianPlanning(arm, target_pose);
            first = false;
        } else {
            cartesianPlanningNext(arm, target_pose);
        }
