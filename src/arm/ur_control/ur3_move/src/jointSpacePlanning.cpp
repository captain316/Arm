...
int main(int argc, char **argv)  //主函数
{
    ...
    moveit_msgs::AttachedCollisionObject camera = attachObject(arm, "camera", "fingers_frame", 0.19, 0.05, 0.05, 0.0, 0.25, 0.763);
    ...
    // double ox = getPose.response.target_pose.orientation.x;
    // double oy = getPose.response.target_pose.orientation.y;
    // double oz = getPose.response.target_pose.orientation.z;
    // double ow = getPose.response.target_pose.orientation.w;
    ...
    angle =  1.5708 + angle;
    // if (angle >= -3.14159 && angle < -1.5708) {
    //     angle += 3.14159;
    // } else if (angle > 1.5708 && angle <= 3.14159) {
    //     angle -= 3.14159;
    // }
    ...
    target_pose = setGoal( px, py, pz,
                           q.getX(), q.getY(), q.getZ(), q.getW() );
    target_pose.position.z += 0.02; // 移动到物体的上方2cm
    ...
    success = arm.plan(plan_2) == moveit::planning_interface::MoveItErrorCode::SUCCESS;
    // 输出成功与否的信息
    ROS_INFO("Plan (pose goal) %s", success ? "" : "FAILED");   
    // 让机械臂按照规划的轨迹开始运动
    if(success) {
        arm.execute(plan_2); // 到达物体抓取位置
    }
